...
ochol_mean = np.mean(data["chol"])
ochol_median = np.median(data["chol"])
ochol_std = np.std(data["chol"])
...
reshaped_data = np.reshape(data.values, (data.shape[0], -1))
splitted_data = np.split(reshaped_data, 2)
